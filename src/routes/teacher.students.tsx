...
function StudentsPage() {
  ...
  if (loading) return <div className="p-6 text-gray-400">Loading students...</div>;

  return (
    <div className="p-4 md:p-6">
      <h1 className="text-2xl font-bold text-white mb-6">Students ({students.length})</h1>
      ...
      <div className="bg-gray-800 rounded-xl overflow-x-auto">
        <table className="w-full min-w-[400px]">
          <thead className="bg-gray-700">
            <tr>
              <th className="text-left text-gray-300 text-sm px-4 py-3">Student</th>
              <th className="text-left text-gray-300 text-sm px-4 py-3 hidden sm:table-cell">Courses</th>
              <th className="text-left text-gray-300 text-sm px-4 py-3 hidden sm:table-cell">Submissions</th>
            </tr>
          </thead>
            ...
            {filtered.map(s => (
              <tr key={s.id} className="hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 hidden sm:table-cell">
                  <span className="bg-blue-900/30 text-blue-400 text-xs px-2 py-0.5 rounded-full">{s.enrollments} courses</span>
                </td>
                <td className="px-4 py-3 hidden sm:table-cell">
                  <span className="bg-green-900/30 text-green-400 text-xs px-2 py-0.5 rounded-full">{s.submissions} submitted</span>
                </td>
              </tr>
            ))}
          </tbody>
